...
NEWS_MAX_AGE_DAYS = 4   # 발행 후 이 일수 이내 기사만 수집 (7일은 너무 좁음)
...
QUERIES = {
    "hr": [
        # 1. 인사/조직/평가 일반
        "(HR OR 인사) (제도 OR 기획 OR 조직문화 OR 성과평가 OR 개편)",
        "(채용 OR 직원경험 OR 몰입도) (트렌드 OR 통계 OR 전략)",
        "핵심인재 (유치 OR 리텐션 OR 이탈 OR 역량개발)",
        # 2. 보상 및 트렌드
        "(이직률 OR 퇴사율) (RSU OR 스톡옵션 OR 보상 OR 통계)",
        "(상대평가 OR 절대평가) (폐지 OR 성과주의 OR 보상)",
        "(퇴직연금 OR 디폴트옵션) 수익률",
        # 3. 근로제도 및 생산성
        "(유연근무 OR 재택근무 OR 주4일제) (생산성 OR 효율 OR 통계 OR 도입)",
        "(인건비 OR 사내복지) (비용 OR 감축 OR 통계 OR 효율)",
        "(구조조정 OR 희망퇴직 OR 정년연장 OR 임금피크제) (도입 OR 인력)",
        # 4. 리스크 관리 (노사/법무)
        "(노동법 OR 근로기준법 OR 중대재해처벌법) (개정 OR 판례 OR 위반)",
        "(임단협 OR 최저임금) (타결 OR 노사 OR 인상)",
        # 5. 경영진 및 데이터 HR
        "(지주사 OR 그룹사) (임원인사 OR CEO 승계 OR 조직개편)",
        "(피플애널리틱스 OR HR데이터 OR HR SaaS) (도입 OR 통계)",
        "ESG 여성 임원 비율"
    ],
    "ai_tech": [
        "AI 기업 생산성 도입",
