...
QUERIES = {
    "hr": [
        # 1. 평가/조직문화/채용 (정치, 정부기관 완벽 차단)
        "기업 (인사제도 OR 조직문화 OR 성과평가 OR 인사평가) -청문회 -장관 -국회 -대통령 -북한 -후보 -공무원 -교육청",
        "기업 (채용 트렌드 OR 직원경험 OR 조직 몰입도 OR HRD) -정치 -선거",
        "기업 (핵심인재 OR 인재영입) (보상 OR 리텐션 OR 유치) -여야 -총선 -영입인재",

        # 2. 보상/평가 트렌드
        "기업 (RSU OR 스톡옵션 OR 성과급 OR 보상체계) (개편 OR 도입 OR 이직률 통계)",
        "기업 (상대평가 OR 절대평가 OR 인사고과) (개편 OR 도입 OR 성과주의)",

        # 3. 근로제도/생산성
        "기업 (유연근무 OR 재택근무 OR 주4일제) (생산성 OR 효율 OR 도입 통계)",
        "기업 (사내복지 OR 복리후생 OR 워라밸) (비용 OR 통계 OR 트렌드)",
        "기업 (구조조정 OR 희망퇴직 OR 임금피크제) (인력 OR 효율화 OR 통계)",

        # 4. 노사관계/리스크
        "기업 (근로기준법 OR 중대재해처벌법 OR 최저임금) (판례 OR 대응 OR 위반) -국회 -여야 -정치",
        "기업 (임단협 OR 임금교섭 OR 노사협상) (타결 OR 합의 OR 갈등) -정치 -공공기관",

        # 5. 그룹사 임원인사 및 HR Tech
        "(지주사 OR 계열사 OR 재계) (조직개편 OR 임원인사 OR 세대교체) -장관 -청문 -대통령 -정부",
        "기업 (피플애널리틱스 OR HR데이터 OR 인사관리 솔루션) 도입",
    ],
    "ai_tech": [
        "AI 기업 생산성 도입",
        "디지털 전환 DX 기업",
